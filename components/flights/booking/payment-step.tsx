...
export function PaymentStep({ bookingData, onComplete, onBack }: PaymentStepProps) {
  return (
    <Elements stripe={stripePromise}>
      <PaymentForm bookingData={bookingData} onComplete={onComplete} onBack={onBack} />
    </Elements>
  );
}
...
function PaymentForm({ bookingData, onComplete, onBack }: PaymentStepProps) {
  ...
  const calculateSubtotal = useCallback(() => {
    ...
    const flightPrice = bookingData.flightDetails?.basePrice || bookingData.flightDetails?.price || 0;
    
    // Passengers count
    const passengerCount = bookingData.passengers?.length || 1;
    
    // Seat upgrades
    const seatUpgradesTotal = bookingData.selectedSeats?.reduce(
  ...
  }, [bookingData.flightDetails, bookingData.passengers, bookingData.selectedSeats, bookingData.addons]);
    ...
    const createBookingAndPaymentIntent = async () => {
      try {
        ...
        if (!storedBookingData || 
          ...
          throw new Error("Incomplete booking data. Please complete all previous steps.");
        }
        ...
        const passengerDetails = storedBookingData.passengers.map(passenger => ({
        ...
        const seatNumbers = storedBookingData.selectedSeats.map(seat => seat.seatNumber);
        // Get the cabin class from the selected seat (first passenger)
        const selectedCabin = bookingData?.selectedSeats?.[0]?.seatClass || "Economy";
        ...
        toast({
          title: "Setup Error",
          description: "Could not initialize booking or payment. Please try again.",
          variant: "destructive",
        });
      }
    };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <CardContent>
              ...
              <div className="mt-6">
                ...
                <div className="space-y-4">
                  <div className="p-3 border rounded-md" data-testid="card-element-container">
                    <CardElement 
                      options={{
                        style: {
                          base: {
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center space-x-2 mb-4">
                <input type="checkbox" id="same-address" className="rounded" checked={true} readOnly />
              ...
              {bookingData.contactInfo?.address && (
                <div className="bg-muted p-4 rounded-md">
                  ...
                  <p>
                    ...
                    {bookingData.contactInfo.address.state && ` ${bookingData.contactInfo.address.state},`} 
        ...
        <div>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between">
                  <span>Flight ({bookingData.passengers?.length || 1} passenger{(bookingData.passengers?.length || 1) > 1 ? 's' : ''})</span>
                  <span>${((bookingData.flightDetails?.basePrice || bookingData.flightDetails?.price || 0) * (bookingData.passengers?.length || 1)).toFixed(2)}</span>
                </div>
                
                {bookingData.selectedSeats && bookingData.selectedSeats.length > 0 && (
                  <div className="flex justify-between">
                    <span>Seat selection</span>
                    <span>${bookingData.selectedSeats.reduce((acc, seat) => acc + seat.price, 0).toFixed(2)}</span>
                  </div>
                )}
                ...
                {bookingData.addons && bookingData.addons.length > 0 && (
                  <div className="flex justify-between">
                    <span>Add-ons</span>
                    <span>${bookingData.addons.reduce((acc, addon) => addon.selected ? acc + addon.price : acc, 0).toFixed(2)}</span>
                  </div>
                )}
          ...
          <div className="mt-4 text-sm text-muted-foreground">
            <p>By clicking &quot;Pay&quot;, you agree to our <a href="#" className="underline">Terms and Conditions</a> and <a href="#" className="underline">Privacy Policy</a>.</p>
          </div>
        </div>
      </div>
